...
function listBanners() { return database.prepare("SELECT id, imagen_url AS image FROM banners_publicitarios WHERE activo = 1 AND imagen_url IS NOT NULL AND imagen_url <> '' ORDER BY id").all(); }
function listBannersAdmin() { return database.prepare("SELECT id, imagen_url AS image FROM banners_publicitarios WHERE activo = 1 ORDER BY id").all(); }
...
function removeDefaultBanners() { database.prepare("DELETE FROM banners_publicitarios WHERE imagen_url IS NULL AND titulo IN ('Soluciones para impulsar tu negocio', 'Atención comercial personalizada', 'Confianza que acompaña tu crecimiento')").run(); }
function initializeAttentionPoints() { if (database.prepare('SELECT id FROM puntos_atencion LIMIT 1').get()) return; const insert = database.prepare('INSERT INTO puntos_atencion (nombre, direccion) VALUES (?, ?)'); [['Tarapoto Tienda', 'Jr. Cabo Alberto Leveau 264, Tarapoto'], ['Tarapoto Tienda', 'Jr. Santa Rosa 481, Tarapoto'], ['Tarapoto Oficina Principal', 'Av. Vía de Evitamiento 1846, Tarapoto'], ['Tarapoto Tienda', 'Av. Illo 601, Tarapoto'], ['Tarapoto Almacén principal', 'Av. Vía de Evitamiento 1590, Tarapoto'], ['Yurimaguas Tienda', 'Jr. José Riera 136, Yurimaguas'], ['Yurimaguas Tienda', 'Calle Monseñor Atanasio Jáuregui 528, Yurimaguas'], ['Yurimaguas Tienda', 'Calle Teniente César López 627, Yurimaguas']].forEach((item) => insert.run(...item)); }
