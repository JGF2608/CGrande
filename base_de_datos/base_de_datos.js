...
function listBanners() { return database.prepare('SELECT id, titulo AS title, descripcion AS description, texto_boton AS buttonText, enlace AS link, imagen_url AS image FROM banners_publicitarios WHERE activo = 1 ORDER BY id').all(); }
function createBanner(data) { const id = Number(database.prepare('INSERT INTO banners_publicitarios (titulo, descripcion, texto_boton, enlace, imagen_url) VALUES (?, ?, ?, ?, ?)').run(data.title, data.description || null, data.buttonText || 'Conocer más', data.link || '#productos', data.image || null).lastInsertRowid); return listBanners().find((item) => item.id === id); }
function updateBanner(id, data) { const result = database.prepare('UPDATE banners_publicitarios SET titulo = ?, descripcion = ?, texto_boton = ?, enlace = ?, imagen_url = ?, fecha_actualizacion = CURRENT_TIMESTAMP WHERE id = ? AND activo = 1').run(data.title, data.description || null, data.buttonText || 'Conocer más', data.link || '#productos', data.image || null, id); return result.changes ? listBanners().find((item) => item.id === id) : null; }
...
function initializeBanners() { if (database.prepare('SELECT id FROM banners_publicitarios LIMIT 1').get()) return; const insert = database.prepare('INSERT INTO banners_publicitarios (titulo, descripcion, texto_boton, enlace) VALUES (?, ?, ?, ?)'); [['Soluciones para impulsar tu negocio', 'Conoce nuestro catálogo y encuentra productos pensados para cada necesidad.', 'Ver productos', '#productos'], ['Atención comercial personalizada', 'Solicita una cotización y recibe una propuesta según lo que necesitas.', 'Solicitar cotización', '#cotizacion'], ['Confianza que acompaña tu crecimiento', 'Más de dos décadas construyendo relaciones comerciales duraderas.', 'Conócenos', '#empresa']].forEach((item) => insert.run(...item)); }
