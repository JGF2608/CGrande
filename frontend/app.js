...
async function initializeAttentionMap(apiKey) { const mapElement = document.querySelector('#attention-map'); const mapMessage = document.querySelector('#attention-map-message'); if (!apiKey) { mapElement.innerHTML = '<p class="map-placeholder">Configura la clave de Google Maps para ver todos los puntos marcados aquí.</p>'; mapMessage.textContent = 'Mientras tanto, puedes abrir cada dirección directamente en Google Maps.'; return; } try { await loadGoogleMaps(apiKey); attentionMap = new google.maps.Map(mapElement, { center: { lat: -6.48, lng: -76.37 }, zoom: 8, mapTypeControl: false, streetViewControl: false, fullscreenControl: true }); attentionInfoWindow = new google.maps.InfoWindow(); const geocoder = new google.maps.Geocoder(); const bounds = new google.maps.LatLngBounds(); const geocodePoint = (point) => new Promise((resolve) => { geocoder.geocode({ address: `${point.address}, Perú` }, (results, status) => { if (status === 'OK' && results[0]) { const position = results[0].geometry.location; const marker = new google.maps.Marker({ map: attentionMap, position, title: point.name }); marker.addListener('click', () => focusAttentionPoint(point.id)); attentionMarkers.set(point.id, { marker, position, point }); bounds.extend(position); } resolve(); }); }); await Promise.all(attentionPoints.map(geocodePoint)); if (!bounds.isEmpty()) { attentionMap.fitBounds(bounds, 48); google.maps.event.addListenerOnce(attentionMap, 'idle', () => attentionMap.setZoom(Math.min(attentionMap.getZoom(), 14))); mapMessage.textContent = 'Selecciona una ubicación para acercarte a ese punto.'; } else mapMessage.textContent = 'No pudimos ubicar las direcciones automáticamente. Usa los enlaces de cada ubicación.'; } catch (error) { mapElement.innerHTML = '<p class="map-placeholder">No fue posible cargar el mapa.</p>'; mapMessage.textContent = error.message; } }
