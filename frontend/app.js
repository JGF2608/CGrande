...
async function loadWhatsAppSettings() { const settings = await fetch('/api/company-settings').then((response) => response.json()); whatsappNumber = settings.whatsappNumber || ''; renderSocialNetworks(settings); }
function googleMapsUrl(point) { return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${point.address}, Perú`)}`; }
function renderAttentionPoints() { const list = document.querySelector('#attention-list'); list.innerHTML = attentionPoints.map((point) => `<article class="attention-point"><h3>${escapeHtml(point.name)}</h3><p>${escapeHtml(point.address)}</p><div><button class="attention-focus" data-id="${point.id}" type="button">Ver en el mapa</button><a href="${googleMapsUrl(point)}" target="_blank" rel="noopener">Abrir en Google Maps ↗</a></div></article>`).join('') || '<p>No hay puntos de atención registrados.</p>'; document.querySelectorAll('.attention-focus').forEach((button) => button.addEventListener('click', () => focusAttentionPoint(Number(button.dataset.id)))); }
...
async function loadAttentionPoints() { attentionPoints = await fetch('/api/attention-points').then((response) => response.json()); renderAttentionPoints(); const configuration = await fetch('/api/maps/config').then((response) => response.json()); await initializeAttentionMap(configuration.apiKey || ''); }
...
function renderBanner() { const slider = document.querySelector('#banner-slider'); const hasManyBanners = bannerItems.length > 1; if (!bannerItems.length) { slider.parentElement.classList.add('hidden'); return; } slider.parentElement.classList.remove('hidden'); const banner = bannerItems[activeBannerItem]; slider.innerHTML = `<img class="banner-slide" src="${banner.image}" alt="Banner publicitario ${activeBannerItem + 1}">`; document.querySelectorAll('.banner-arrow').forEach((button) => { button.classList.toggle('hidden', !hasManyBanners); }); }
...
Promise.all([loadWhatsAppSettings(), loadBanners(), loadProducts(), loadKpis(), loadTimeline(), loadAttentionPoints()]);
