...
async function loadBanners() { bannerItems = await fetch('/api/banners').then((response) => response.json()); activeBannerItem = 0; renderBanner(); }
function renderBanner() { const slider = document.querySelector('#banner-slider'); const controls = document.querySelector('.banner-controls'); if (!bannerItems.length) { slider.parentElement.classList.add('hidden'); return; } const banner = bannerItems[activeBannerItem]; const image = banner.image ? `style="background-image:linear-gradient(90deg,#09281be8 0%,#09281b8a 57%,#09281b40 100%),url('${banner.image}')"` : ''; const action = banner.link ? `<a class="button banner-action" href="${escapeHtml(banner.link)}">${escapeHtml(banner.buttonText || 'Conocer más')} <span aria-hidden="true">→</span></a>` : ''; slider.innerHTML = `<article class="banner-slide ${banner.image ? 'has-image' : ''}" ${image}><div><p class="eyebrow">NOVEDADES</p><h2>${escapeHtml(banner.title)}</h2><p>${escapeHtml(banner.description || '')}</p>${action}</div></article>`; document.querySelector('#banner-dots').innerHTML = bannerItems.map((item, index) => `<button class="banner-dot ${index === activeBannerItem ? 'active' : ''}" type="button" aria-label="Ver banner ${index + 1}"></button>`).join(''); document.querySelectorAll('.banner-dot').forEach((button, index) => button.addEventListener('click', () => { activeBannerItem = index; renderBanner(); })); controls.classList.toggle('hidden', bannerItems.length < 2); }
function moveBanner(direction) { activeBannerItem = (activeBannerItem + direction + bannerItems.length) % bannerItems.length; renderBanner(); }
